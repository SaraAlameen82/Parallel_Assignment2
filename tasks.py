...
@app.task
def a_star_task(width, height, maze_type, visualize=False):
    """
...
@app.task
def bfs_task(width, height, maze_type, visualize=False):
    """
    ...
    time_taken, moves = bfs_explorer(maze_copy, visualize)
    return {
        'time_taken': time_taken,
        'moves': len(moves),
        'type': maze_type
